...
type Discover interface {
	...
	// InstanceId 获取服务instanceId
	InstanceId(serviceName string) string
}
...
func (discoverClient *DiscoverClient) Register(serviceName string, instanceId string, instanceHost string, instancePort int, mete map[string]string, healthUrl string)bool{
	//生成一个实例
...
func (discoverClient *DiscoverClient) DeRegister(instanceId string)bool{
	serviceRegistration := &api.AgentServiceRegistration{
		ID: instanceId,
	}
...
func (discoverClient *DiscoverClient) DiscoverServices(serviceName string)[]interface{} {
	if serviceName == "" {
		return []interface{}{}
	}
...
//InstanceId 获取服务instanceId
func (discoverClient *DiscoverClient)InstanceId(serviceName string) string  {
	return serviceName + "-" +  uuid.NewV4().String()
}
